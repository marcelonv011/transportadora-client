    ...
    <div class="column is-10 is-offset-1">
      <div v-for="item in freteList" :key="item.id">
        <div v-if="item.statusFrete === 'EM_TRANSPORTE'">
          ...
          <h1>
            Voce esta querendo mudar para o estado do frete em interrompido
          </h1>
        </div>
      </div>
      <textarea
        class="textarea is-primary"
        placeholder="deixe uma observacao porque quer mudar para interrompido"
...
@Component
export default class InterrompidoTransporteFrete extends Vue {
  ...
  public onclickActualizar(): void {
    this.freteClient.atualizarStatusCancelado(this.frete).then(
      (success) => {
